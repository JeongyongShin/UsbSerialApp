...
public class SerialConnector {
	...
	private ArduinoControllerActivity.SerialListener mListener;
	...
	******************************************************/
	public SerialConnector(Context c, ArduinoControllerActivity.SerialListener l, Handler h) {

		Log.i("App-Serial", "SerialConnector( ) start !! ");
		mContext = c;
		mListener = l;
		mHandler = h;
	}
	...
	public void initialize() {
		...
		Log.i("App-Serial", "initialize( ) start !! ");
		...
		Log.i("App-Serial", "initialize( ) --> getDevice() device : " +device.getDeviceName()+  ", deviceID :" +device.getDeviceId() + ", vendorID :" +device.getVendorId());
		...
		Log.i("App-Serial", "initialize( ) --> getPorts() , mPort : " +mPort );
		if(mPort == null) {
		...
		try {
			...
			Log.i("App-Serial", "initialize( ) --> open() , connection : " +connection );
			mPort.setParameters(115200, 8, 1, 0);		// baudrate:9600, dataBits:8, stopBits:1, parity:N
			Log.i("App-Serial", "mPort.open( )  ");
	...
	public void sendCommand(String cmd) {
		...
		if(mPort != null && cmd != null) {
			try {
				...
				Log.i("App-Serial", "sendCommand( ) , command : " +cmd);
	...
	public class SerialMonitorThread extends Thread {
		...
		**/
		...
		public void run() 
		{
			...
			Log.i("App-Serial", "SerialMonitorThread( ) , run .. ");
